...
export interface IInvestigation extends Omit<Investigation, 'id' | 'patient' | 'doctor' | 'tests'>, Document {
  id: string;
  ...
  toJSON: {
    transform: function(doc, ret) {
      ...
      if (ret.testIds && Array.isArray(ret.testIds)) {
        ret.testIds = ret.testIds.map((id: any) => 
...
InvestigationSchema.pre('save', async function(next) {
  if (this.isNew) {
    try {
      ...
      const maxOrderInStatus = await Investigation
        .findOne({ status: this.status })
        .sort({ order: -1 })
        .select('order');
